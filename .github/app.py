...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'meetpro-2026-' + uuid.uuid4().hex
CORS(app)

# ✅ Changed to eventlet for production (Railway + gunicorn)
...
# ── In-memory state ──────────────────────────────────────────
rooms        = {}   # room_id -> {participants, chat, created_at}
user_sockets = {}   # user_id -> socket_id
socket_users = {}   # socket_id -> {userId, roomId}
...
def on_join(data):
    ...
    if len(room['participants']) >= 50:
        emit('error', {'message': 'Room is full (50/50). Please try another room.'})
        return
...
def on_raise_hand(data):
    ...
    if room_id in rooms and user_id in rooms[room_id]['participants']:
        rooms[room_id]['participants'][user_id]['handRaised'] = raised
...
if __name__ == '__main__':
    ...
    print(f"\n{'='*52}")
    print(f"  🎥  MeetPro — Video Meeting Server")
    print(f"{'='*52}")
    print(f"  Local  : http://localhost:{port}")
    print(f"  Network: http://0.0.0.0:{port}")
    print(f"  Max participants per room: 50")
    print(f"{'='*52}\n")
